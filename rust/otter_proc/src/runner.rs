///// Otter: Prozessstart, Streaming-Logs & ephemere Progress-Zeile (Spinner, ETA, % aus Metrics & Builder); ASCII-only (+ ANSI-Farbe aktiv).
///// Schneefuchs: Fehlercodes sauber weiterreichen; CWD optional; ENV-Overlay; .build_metrics zentral über build_metrics.rs (atomar, Seeding). Warn-/Error-Triage farbig.
/// ///// Maus: Ruhig bei Output-Dauerfeuer, 1 Hz-Throttle; keine externen Crates; klare Metrics-Logs (metrics=…, metrics-seeded-from=…).
...
// --- simple tagged output helpers (ASCII only + color tags) ------------------
...
// ---- progress helpers (local, no external deps) -----------------------------
...
fn progress_enabled() -> bool {
    // Default: enabled. Disable with OTTER_NO_PROGRESS=1 (not exposed as flag).
...
fn detect_phase_and_sig(exe: &str, args: &[String]) -> PhaseDetect {
    ...
    for i in 0..args.len() {
        if args[i] == "--config" && i + 1 < args.len() {
            cfg = Some(args[i + 1].clone());
        } else if args[i] == "--preset" && i + 1 < args.len() {
            preset = Some(args[i + 1].clone());
        } else if args[i] == "--build" {
            phase = "build".to_string();
        }
    }

    if exe.eq_ignore_ascii_case("cmake") && phase != "build" {
        // assume configure/generate
    ...
    let sig = if exe.eq_ignore_ascii_case("cmake") {
        ...
        // generic signature: exe + first tokens (bounded)
...
pub fn run_streamed_with_env(
    ...
    // Enable ANSI colors (best-effort on Windows).
    enable_ansi();
    ...
    if let Some(src) = seed_src {
        println!("[RUNNER] metrics-seeded-from={}", src.display());
    }
    ...
    let mut out_reader = match child.stdout.take() {
        ...
        None => {
            out_err("RUN", "failed to take stdout");
            return RunResult { code: 1 };
        }
    };
    let mut err_reader = match child.stderr.take() {
        Some(s) => BufReader::new(s),
        None => {
            out_err("RUN", "failed to take stderr");
            return RunResult { code: 1 };
        }
    };
    ...
    loop {
        ...
        if let Ok(n) = out_reader.read_line(&mut out_buf) {
            if n > 0 {
                // clear ephemeral first if needed
                ...
                match classify_line(&out_buf) {
                    Sev::Err  => { eprint!("{}[ERR]{}   [{}] {}",  RED, RESET, tag, out_buf); let _ = std::io::stderr().flush(); }
                    Sev::Warn => { println!("{}[WARN]{}  [{}] {}", YEL, RESET, tag, out_buf); let _ = std::io::stdout().flush(); }
                    Sev::Info => { println!("[INFO]  [{}] {}", tag, out_buf); let _ = std::io::stdout().flush(); }
                }

                last_snippet = last_nonempty_snippet(&out_buf, 80);
                progressed = true;
            }
        }
        ...
        if let Ok(n) = err_reader.read_line(&mut err_buf) {
            if n > 0 {
                ...
                match classify_line(&err_buf) {
                    Sev::Err  => { eprint!("{}[ERR]{}   [{}] {}",  RED, RESET, tag, err_buf); let _ = std::io::stderr().flush(); }
                    Sev::Warn => { println!("{}[WARN]{}  [{}] {}", YEL, RESET, tag, err_buf); let _ = std::io::stdout().flush(); }
                    Sev::Info => { println!("[INFO]  [{}] {}", tag, err_buf); let _ = std::io::stdout().flush(); }
                }

                let snip = last_nonempty_snippet(&err_buf, 80);
        ...
        if !progressed {
            // emit ephemeral progress (1 Hz) if enabled
            if progress_enabled() && last_tick.elapsed() >= Duration::from_secs(1) {
                ...
                // Progress farbig (dezent cyan) – Inhalt bleibt ASCII, am Ende resetten
                print!("\r[PROG]  [{}] {}{} {}  pred={}s  {}  {}{}",
