///// Otter: GT-Palette (Cyan→Amber) integriert; keine neuen Dateien; Signaturen unverändert.
///?? Schneefuchs: Host/Device sauber getrennt; keine Device-Intrinsics im Hostcode; ASCII-Logs.
///?? Maus: Sofort kompilierbar; nur Farbpfad geändert; Smooth-Iterations gegen Banding.
...
// ============================================================================
// Kernel – Perturbation-Pfad (vereinfacht; nutzt Referenz-Orbit + Delta)
// ============================================================================
    ...
    int w, int h, float zoom, float2 center, float /*deltaRelMax*/)
{
    ...
    int it = 0;
    const float bailout2 = 64.0f; // (8)^2

    #pragma unroll 1
    for (int n=0; n<maxIter; ++n) {
        const float2 ref = refOrbit[n];
        const float2 dz2     = mul(z, z);
        const float2 twoRefDz= muls(mul(ref, z), 2.0f);
        z = add(add(dz2, twoRefDz), c);
        ++it;
        if (dot2(z) > bailout2) break;

        // occasional safety: fall back to plain iteration if needed
        if ((n & 63) == 63) {
            float zx = z.x, zy = z.y;
...
ESCAPE:
    {
        const uchar4 col = gtColor_fromSmoothState(it, maxIter, z.x, z.y);
        out[idx] = col;
        iterOut[idx] = it;
    }
}
    ...
    int maxIter)
{
    ...
    // Reference orbit around center (host, float) – simple & fast
    ...
    hostOrbit.resize(maxIter);
    {
        ...
        for (int i=0; i<maxIter; ++i){
            ...
            // NOTE: host side must not use device intrinsics → use std::fma
