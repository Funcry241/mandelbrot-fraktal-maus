...
constexpr float kTHETA_DAMP_LO  = 0.42f;  // ~24 deg
constexpr float kTHETA_DAMP_HI  = 1.32f;  // ~75 deg
...
// Trig-free rotate-towards with bounded turn (angle approx sqrt(2*(1 - dot)))
    ...
    for(int ty=0;ty<tilesY;++ty){
        for(int tx=0;tx<tilesX;++tx){
            ...
            for(int k=0;k<4;++k){
                ...
                sum += std::fabs(entropy[ny[k]*tilesX+nx[k]]-c); ++n;
            }
    ...
    ZoomState& state) noexcept
{
    ...
    if (g_inHandover && inFreeze) {
        g_handoverT = 0.0f; // still in freeze
    ...
    e.clear(); e.reserve((size_t)totalTiles);
    c.clear(); c.reserve((size_t)totalTiles);
    ...
    std::array<float, kMIN_TOPK> topK;
    for (auto& v : topK) { v = -1e9f; }

    auto pushTopK = [&](float s){
        size_t minPos = 0;
        for (size_t i=1;i<topK.size();++i){
            if (topK[i] < topK[minPos]) { minPos = i; }
        }
    ...
    float topKMin = topK[0];
    for(size_t i=1;i<topK.size();++i){ topKMin = std::min(topKMin, topK[i]); }
    ...
    if (g_dirInit) {
        ...
        if (normalize2D(nx,ny)) {
            ...
            // Deadband: <~3 deg -> keep old direction
            if (cosA > 0.9986f) { // cos(3 deg)
                ndcTX = g_prevDirX; ndcTY = g_prevDirY;
            } else {
                // Map cosA in [~0.906, 0.996] (~25..5 deg) -> wOld in [0.15, 0.50]
    ...
    // Handover blend right after freeze ends (blend seed->proposed for ~300 ms)
    if (g_inHandover) {
    ...
    if constexpr (Settings::debugLogging) {
        const double ndcLen = std::sqrt((double)ndcTX*(double)ndcTX + (double)ndcTY*(double)ndcTY);
