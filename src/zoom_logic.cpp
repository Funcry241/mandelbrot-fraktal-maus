///// src/zoom_logic.cpp — V3-lite
// Fokus: ruckelfreie Richtungswechsel (Turn-Limiter + Längendämpfung),
// kein "in place"-Zoom (Warm-up-Drift + Seed-Step), schlanke Softmax-Zielwahl.
// Keine OMP-/Cache-Komplexität. Nur ASCII-Logs wenn Settings::debugLogging.
...
constexpr float kMIN_SIGNAL_STD = 0.15f; // Minimum-Std-Abweichung der Scores für "aktives" Signal
...
inline void antiVoidDriftNDC(float cx, float cy, float& ndcX, float& ndcY) {
    ...
    float vx  = vx1 + 0.8f * vx2;        // leicht stärkere Rechtspräferenz
    float vy  = vy1 + 0.8f * vy2;
    ...
    ZoomState& state) noexcept
{
    ...
    const int totalTiles = tilesX*tilesY; if(tilesX<=0||tilesY<=0||totalTiles<=0 || (int)entropy.size()<totalTiles || (int)contrast.size()<totalTiles){
    ...
    if (freezeDirection) {
        if (insideCardioidOrBulb(currentOffset.x, currentOffset.y)) {
            ...
            const float2 target = make_float2(previousOffset.x + nx * (float)(kWARMUP_DRIFT_NDC*invZoom),
                                              previousOffset.y + ny * (float)(kWARMUP_DRIFT_NDC*invZoom));
            const float a=0.20f; out.newOffset = make_float2(previousOffset.x*(1.0f-a)+target.x*a,
        ...
        out.shouldZoom = true; return out;
    }
    ...
    // Signalstärke ~ Std-Abweichung der Scores (ungefähr)
    double meanS=0.0, meanS2=0.0; for(int i=0;i<totalTiles;++i){ const float ez=(entropy[i]-eMed)/eMad; const float cz=(contrast[i]-cMed)/cMad; const float s=kALPHA_E*ez+kBETA_C*cz; meanS+=s; meanS2+=double(s)*s; }
    meanS/=std::max(1, totalTiles); const double varS = std::max(0.0, meanS2/std::max(1,totalTiles) - meanS*meanS); const double stdS = std::sqrt(varS);
    ...
    for (int i=0;i<totalTiles;++i){
        ...
        const double cx = currentOffset.x + ndcX[i]*invZoom; const double cy = currentOffset.y + ndcY[i]*invZoom; if (insideCardioidOrBulb(cx,cy)) continue;
        const double w = std::exp(double((s - bestScore)*invTemp)); sumW += w; numX += w*ndcX[i]; numY += w*ndcY[i]; if (s>bestAdjScore){ bestAdjScore=s; bestAdj=i; }
    }
    ...
    if (sumW>0.0){ const double inv = 1.0/sumW; ndcTX = numX*inv; ndcTY = numY*inv; }
        ...
        const double cx = currentOffset.x + ndcX[bestIdx]*invZoom; const double cy = currentOffset.y + ndcY[bestIdx]*invZoom;
    ...
    const float2 rawTarget = make_float2(previousOffset.x + (float)(ndcTX*invZoom),
                                         previousOffset.y + (float)(ndcTY*invZoom));
    float mvx = rawTarget.x - previousOffset.x; float mvy = rawTarget.y - previousOffset.y; const float rawDist = std::sqrt(mvx*mvx + mvy*mvy);
    ...
    float lenScale = 1.0f;
    if (hasMove){
        const float preDot = clampf(dirX*tgtX + dirY*tgtY, -1.0f, 1.0f); const float preAng = std::acos(preDot);
    ...
    out.bestIndex = bestAdj >= 0 ? bestAdj : bestIdx;
    ...
    if (Settings::debugLogging) {
        LUCHS_LOG_HOST("[ZOOM-LITE] stdS=%.3f hasSig=%d turnMax=%.3f lenScale=%.3f ema=%.3f dist=%.4f idx=%d",
                       (float)stdS, hasSignal?1:0, maxTurn, lenScale, emaAlpha, out.distance, out.bestIndex);
    }
