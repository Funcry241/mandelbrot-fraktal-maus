...
void renderCudaFrame(
    ...
    try {
        ...
        // [LOG-1b] Deterministisches Pointer/Size-Logging nach erfolgreichem Map
        if constexpr (Settings::debugLogging) {
        ...
        ::computeCudaEntropyContrast(
            ...
            state.evEcDone              // record done-event (optional)
        );
        ...
        // Maus: Auf JEDEN CUDA-Fehler sofort Device-Log flushen (best effort).
        // (Auch bei anderen Fehlerarten harmlos – kein Host-Log-Spam.)
        LuchsLogger::flushDeviceLogToHost(0);
        throw;
    }
}
