...
// Zeilen: 293
// 🐭 Maus-Kommentar: Capybara+Kiwi+MausZoom – Kompaktierung: Logik 100% erhalten, OOB & Testwerte garantiert, Mappingbug final eliminiert. Otter sagt: Jeder Buffer respektiert! Schneefuchs: Kein Kompromiss, keine Extrazeile!
...
__global__ void mandelbrotKernelAdaptive(uchar4* out, int* iterOut, int w, int h, float zoom, float2 offset, int maxIter, int tile, int* super) {
    ...
    for (int i = 0; i < S; ++i) for (int j = 0; j < S; ++j) {
        ...
        float norm = zx*zx+zy*zy, tt = (it + 1.0f - log2f(log2f(fmaxf(norm, 1e-8f)))) / maxIter;
...
__global__ void entropyKernel(const int* it, float* eOut, int w, int h, int tile, int maxIter) {
    ...
    if (threadIdx.x==0) {
        ...
        for(int i=0;i<256;++i){float p=(local>0?histo[i]/float(total):0.0f);if(p>0.0f)entropy-=p*log2f(p);used+=histo[i];}
        int tilesX=(w+tile-1)/tile, tileIndex=tY*tilesX+tX; eOut[tileIndex]=(used>0)?entropy:0.0f;
    }
}
...
void launch_mandelbrotHybrid(uchar4* out, int* d_it, int w, int h, float zoom, float2 offset, int maxIter, int tile, int* d_sup, int supersampling) {
    ...
    if(Settings::debugLogging)std::printf("[DEBUG] Mandelbrot-Kernel Call: width=%d, height=%d, maxIter=%d, zoom=%.2f, offset=(%.10f,%.10f), tileSize=%d, supersampling=%d, block=(%d,%d), grid=(%d,%d)\n",w,h,maxIter,zoom,offset.x,offset.y,tile,(d_sup?-42:1),block.x,block.y,grid.x,grid.y);
    mandelbrotKernelAdaptive<<<grid,block>>>(out,d_it,w,h,zoom,offset,maxIter,tile,d_sup);
    cudaError_t err = cudaGetLastError(); if(err!=cudaSuccess)std::fprintf(stderr,"[CUDA ERROR] Kernel launch failed: %s\n",cudaGetErrorString(err)); cudaDeviceSynchronize();
