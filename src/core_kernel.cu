// MAUS:
// Otter: Brightened tone mapping + gentler DE-AA.
// Otter: Pure math image; no patterns, no hotspots; logs ASCII/English.
...
// ----------------------- visual response (pure math) -------------------------
constexpr float AA_K       = 1.2f;  // gentler DE-AA (brighter edges)
constexpr float EXPOSURE   = 1.60f; // lift lows/mids
constexpr float LIFT       = 0.06f; // shadow floor
constexpr float TONE_GAMMA = 0.75f; // gamma brighten
...
// monochrome tone mapping (brightened)
__device__ __forceinline__ float3 shade_mono(int it, int maxIter, float zx, float zy) {
    ...
    float v  = 1.0f - __expf(-EXPOSURE * t);
    v = powf(clamp01(v), TONE_GAMMA);
    v = v + LIFT * (1.0f - v);
    if (kGamma != 1.0f) { const float invG = 1.0f / kGamma; v = powf(clamp01(v), invG); }
    return make_float3(v, v, v);
}

// DE-based AA coverage with floor
__device__ __forceinline__ float coverage_from_de(float r2, float dx, float dy, float pixR) {
    ...
    float cov = clamp01(de / fmaxf(AA_K * pixR, 1e-30f));
    cov = cov * cov * (3.0f - 2.0f * cov);
    const float COV_FLOOR = 0.35f;
    return fmaxf(cov, COV_FLOOR);
}
    ...
    float& dx, float& dy)
{
    ...
    int it = 0; interiorFlag = false;

    float px = x, py = y; int pc = 0, close_hits = 0;
...
#pragma unroll 1
        for (int s = 0; s < 64; ++s) {
            ...
            if (pc >= 48) {
                ...
                if (d2 < 2e-7f) { if (++close_hits >= 1) { active=false; interiorFlag=true; it=maxSteps; } }
                else close_hits = 0;
                px = x; py = y; pc = 0;
            }
...
__device__ __forceinline__ SliceResult iterate_finish_slice(
    ...
    int sliceSteps)
{
    if (insideMainCardioidOrBulb(cr, ci)) return { maxIter, x, y, dx, dy, false, true };

    int it = start_it; float px = x, py = y; int pc = 0, close_hits = 0, deep_hits = 0;
    unsigned mask = 0xFFFFFFFFu;
#if (__CUDA_ARCH__ >= 700)
    mask = __activemask();
#endif
...
#pragma unroll 1
        for (int s = 0; s < 64; ++s) {
            ...
            if (pc >= 16) {
                float ex = x - px, ey = y - py;
                float d2 = ex*ex + ey*ey;
                if (d2 < 1e-6f) { if (++close_hits >= 1) { active=false; interior=true; it=maxIter; } }
                else close_hits = 0;
                if (d2 < 1e-10f && (x2 + y2) < 4.0f) { if (++deep_hits >= 3) { active=false; interior=true; it=maxIter; } }
                else deep_hits = 0;
                px = x; py = y; pc = 0;
            }
    ...
    int maxIter, float pixR)
{
    ...
    if (insideMainCardioidOrBulb(c.x, c.y)) { out[idx]=make_uchar4(0,0,0,255); iterOut[idx]=maxIter; return; }
    ...
    if (interior) { out[idx]=make_uchar4(0,0,0,255); iterOut[idx]=maxIter; return; }
    ...
    if (escaped) {
        ...
        out[idx]=make_uchar4((unsigned char)(255.0f*clamp01(col.x)),
                             (unsigned char)(255.0f*clamp01(col.y)),
                             (unsigned char)(255.0f*clamp01(col.z)),255);
        iterOut[idx]=itWarm; return;
    }
...
#endif
    const unsigned ballot = __ballot_sync(actMask, true);
    ...
    int w, int h)
{
    ...
    Survivor s = survIn[t];
    if (insideMainCardioidOrBulb(s.cr, s.ci)) { out[s.idx]=make_uchar4(0,0,0,255); iterOut[s.idx]=maxIter; return; }
    ...
    if (r.escaped) {
        ...
        col.x *= cov; col.y *= cov; col.z *= cov;
        out[s.idx]=make_uchar4((unsigned char)(255.0f*clamp01(col.x)),
                               (unsigned char)(255.0f*clamp01(col.y)),
                               (unsigned char)(255.0f*clamp01(col.z)),255);
        iterOut[s.idx]=r.it; return;
    }

    if (r.it >= maxIter || r.interior) { out[s.idx]=make_uchar4(0,0,0,255); iterOut[s.idx]=r.it; return; }
    ...
    const unsigned laneMask = ballot & ((1u << lane) - 1u);
    const int prefix = __popc(laneMask);
    int base = 0; const int leader = __ffs(ballot) - 1;
...
void computeCudaEntropyContrast(const int* d_it, float* d_e, float* d_c, int w, int h, int tile, int maxIter) {
    ...
    if (Settings::performanceLogging || Settings::debugLogging) {
        ...
        if (Settings::performanceLogging) LUCHS_LOG_HOST("[PERF] entropy=%.3f ms contrast=%.3f ms", ms1, ms2);
        else                              LUCHS_LOG_HOST("[TIME] Entropy %.3f ms | Contrast %.3f ms", ms1, ms2);
...
namespace {
    ...
    struct Survivor;
    struct DevicePools { Survivor* A=nullptr; Survivor* B=nullptr; int* cntA=nullptr; int* cntB=nullptr; size_t cap=0; };
    DevicePools g_pools; double g_prevSurvivorsPct = -1.0;
    ...
    double frameBudgetMsFromSettings() {
        if (Settings::capFramerate && Settings::capTargetFps > 0) return std::max(MIN_BUDGET_MS, 1000.0 / double(Settings::capTargetFps));
...
void launch_mandelbrotHybrid(uchar4* out, int* d_it, int w, int h, float zoom, float2 offset, int maxIter, int /*tile*/) {
    ...
    if (Settings::performanceLogging) LUCHS_LOG_HOST("[PERF] warmup_it=%d prev_survivors=%.2f%%", warmupIt, g_prevSurvivorsPct);
    ...
    int h_survA = 0;
    cudaMemcpy(&h_survA, g_pools.cntA, sizeof(int), cudaMemcpyDeviceToHost);
    const double survPct = (double)h_survA * 100.0 / (double(w) * double(h));
    if (Settings::performanceLogging) LUCHS_LOG_HOST("[PERF] survivors=%d (%.2f%% of %d)", h_survA, survPct, w*h);
    ...
    int threads = 128, slice = 0, sliceIt = FINISH_SLICE_IT;
    Survivor* curBuf = g_pools.A; Survivor* nxtBuf = g_pools.B;
    int* curCnt = g_pools.cntA;   int* nxtCnt = g_pools.cntB; int h_cur = h_survA;
    float emaDrop = 0.2f;

    while (h_cur > 0 && slice < 64) {
        double elapsedMs = duration<double, std::milli>(clk::now() - hostStart).count();
        if (elapsedMs >= kernelBudgetMs) { if (Settings::performanceLogging) LUCHS_LOG_HOST("[PERF] budget_exhausted before slice %d", slice); break; }
        cudaMemset(nxtCnt, 0, sizeof(int));
        int blocks = (h_cur + threads - 1) / threads;

        mandelbrotPass2Slice<<<blocks, threads>>>(out, d_it, curBuf, h_cur, nxtBuf, nxtCnt, maxIter, sliceIt, pixR, w, h);
        ...
        if (elapsedMs >= kernelBudgetMs) { if (Settings::performanceLogging) LUCHS_LOG_HOST("[PERF] budget_stop at slice %d", slice); std::swap(curBuf,nxtBuf); std::swap(curCnt,nxtCnt); h_cur=h_next; ++slice; break; }
        ...
        if (emaDrop < DROP_LOWER_ACCEL && sliceIt < (maxIter / 2)) { sliceIt = std::min(sliceIt * 2, maxIter / 2); if (Settings::performanceLogging) LUCHS_LOG_HOST("[PERF] adapt_slice_it=%d (emaDrop=%.4f)", sliceIt, emaDrop); }
        else if (emaDrop > DROP_UPPER_BACKOFF && sliceIt > FINISH_SLICE_IT) { sliceIt = std::max(sliceIt / 2, FINISH_SLICE_IT); if (Settings::performanceLogging) LUCHS_LOG_HOST("[PERF] backoff_slice_it=%d (emaDrop=%.4f)", sliceIt, emaDrop); }

        std::swap(curBuf, nxtBuf); std::swap(curCnt, nxtCnt);
        h_cur = h_next; ++slice;
    }

    if (Settings::performanceLogging || Settings::debugLogging) {
        double totalMs = duration<double, std::milli>(clk::now() - hostStart).count();
        if (Settings::performanceLogging) LUCHS_LOG_HOST("[PERF] mandelbrot (hybrid-sliced): total=%.3f ms (budget=%.3f ms)", totalMs, kernelBudgetMs);
        else                              LUCHS_LOG_HOST("[TIME] Mandelbrot Sliced | Total %.3f ms", totalMs);
    }
}
