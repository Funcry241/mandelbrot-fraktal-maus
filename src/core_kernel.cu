...
// =============================== core_kernel.cu ===============================
// 2-pass Mandelbrot (Warmup + Sliced Finish) — stripped visual FX.
// Features kept:
// - Smooth iteration (continuous potential) shading (math-only).
// - DE-based edge coverage AA (with bright floor).
// - Sliced survivor finish under a kernel-time budget.
// - Entropy/contrast tiles for navigation (not drawn here).
...
// Loop detection thresholds (used in P1/P2)
...
constexpr float AA_K       = 1.15f; // DE-AA scale
constexpr float EXPOSURE   = 1.75f; // tonemap gain
constexpr float LIFT       = 0.08f; // shadow floor
constexpr float TONE_GAMMA = 0.80f; // <1 brightens
constexpr float COV_FLOOR  = 0.58f; // AA coverage floor
...
// Analytic gold ramp from smooth iteration, no stripes, no textures.
__device__ __forceinline__ float3 shade_gold(int it, int maxIter, float zx, float zy) {
    ...
    // gold-ish mapping (monotone per channel)
    float r = clamp01(1.15f * powf(v, 0.42f));
    float g = clamp01(0.95f * powf(v, 0.56f));
    float b = clamp01(0.70f * powf(v, 0.88f));

    // subtle warm bias towards highlights
    r = clamp01(r + 0.06f * (1.0f - v));
    g = clamp01(g + 0.02f * (1.0f - v));

    if (kGamma != 1.0f) {
        const float invG = 1.0f / kGamma;
        r = powf(r, invG); g = powf(g, invG); b = powf(b, invG);
    }
    return make_float3(r, g, b);
}

// DE-based coverage for AA blending, biased brighter at edges.
            ...
            if (pc >= P1_LOOP_EVERY) {
                ...
                if (d2 < P1_LOOP_EPS2) {
                    if (++close_hits >= P1_LOOP_REQ_HITS) {
                        active = false; interiorFlag = true; it = maxSteps;
                    }
                } else close_hits = 0;
                px = x; py = y; pc = 0;
            }
    ...
    int sliceSteps)
{
    ...
    int it = start_it;
    float px = x, py = y; int pc = 0, close_hits = 0, deep_hits = 0;
    ...
    int maxIter, float pixR)
{
    ...
    if (escaped) {
        ...
        float3 col = shade_gold(itWarm, maxIter, zx, zy);
        float aaGain = 0.5f + 0.5f * cov; // bright-biased AA
        col.x *= aaGain; col.y *= aaGain; col.z *= aaGain;
    ...
    int w, int h)
{
    ...
    if (r.escaped) {
        ...
        float3 col = shade_gold(r.it, maxIter, r.x, r.y);
        float aaGain = 0.5f + 0.5f * cov;
        col.x *= aaGain; col.y *= aaGain; col.z *= aaGain;
    ...
    int w, int h, int tile, int maxIter)
{
    ...
    // tilesY removed (unused) to silence warnings
